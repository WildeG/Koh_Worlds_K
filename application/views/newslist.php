...
<?php $i=0 ?>
<?php for($u=5*$_GET['page']; ;$u++){ if (isset($newss[$u])){ ?>
	<div id='news'>
	...
	<h4><?php echo $newss[$u]['title']; ?></h4>
		<table id='table_news'>
			...
			<tr>
				<td colspan='2'><a id='contents'><?php echo join(' ', array_slice(explode( ' ', $newss[$u]['texts'] ), 0, 15)); echo '..'; ?></a></td>
			</tr>
		</table>
	</div>	
...
<?php while ($i<=intval(count($newss)/5)){
	echo '<a href="newslist?page='.$i.'">'.$i.'</a>';
	$i++;
}?>
</center>